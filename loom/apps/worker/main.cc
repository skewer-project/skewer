...
static int RunBatchMode() {
    ...
    const char* frames_per_task_env = std::getenv("LOOM_FRAMES_PER_TASK");
    ...
    const int frames_per_task = frames_per_task_env ? std::atoi(frames_per_task_env) : 1;
    const int fallback_num_frames = task_index * frames_per_task + frames_per_task;
    const int num_frames = num_frames_env ? std::atoi(num_frames_env) : fallback_num_frames;
    ...
    std::cout << "[LOOM BATCH]: Chunk " << task_index << " | frames " << frame_start << "-"
              << frame_end << " | " << prefixes.size() << " layers\n";
    ...
    for (int frame = frame_start; frame <= frame_end; ++frame) {
        ...
        try {
            ...
            exrio::WriteFlatOutputs(flat_image, output_path, /*flatOutput=*/true,
                                    /*pngOutput=*/true, width, height);

            std::cout << "[LOOM BATCH]: Composite complete: " << output_path << "\n";
        } catch (const std::exception& e) {
            std::cerr << "[LOOM BATCH]: Composite failed for frame " << frame << ": " << e.what()
                      << "\n";
            return 1;
        }
    }
