...
void generateRandomFile(int channelCount, Compression compression, bool bulkWrite,
                        bool relativeCoords, const std::string& filename) {
    if (relativeCoords) assert(bulkWrite == false);

    cout << "generating " << flush;
    header = Header(displayWindow, dataWindow, 1, IMATH_NAMESPACE::V2f(0, 0), 1, INCREASING_Y,
                    compression);
    ...
    frameBuffer.insertSampleCountSlice(Slice(IMF::UINT, (char*)(&sampleCount[0][0] - memOffset),
                                             sizeof(unsigned int) * 1, sizeof(unsigned int) * width,
                                             1, 1, 0, relativeCoords, relativeCoords));
    ...
    for (int i = 0; i < channelCount; i++) {
        ...
        frameBuffer.insert(str, DeepSlice(type, (char*)(&data[i][0][0] - memOffset),
                                          pointerSize * 1, pointerSize * width, sampleSize, 1, 1, 0,
                            ...
                            for (int y = box.min.y; y <= box.max.y; y++)
                                for (int x = box.min.x; x <= box.max.x; x++)
                                    for (int k = 0; k < channelCount; k++) {
                                        ...
                                        if (channelTypes[k] == 1) delete[] (half*)data[k][ty][tx];
                                        if (channelTypes[k] == 2) delete[] (float*)data[k][ty][tx];
                                    }
                        }
                    }
                }
    ...
    for (int l = 0; l < sampleCount; l++) {
        if (channelType == 0) {
            ...
            if (value[l] != static_cast<unsigned int>((dwy * width + dwx) % 2049))
                cout << dwx << ", " << dwy << " error, should be " << (dwy * width + dwx) % 2049
                     << ", is " << value[l] << endl
                     << flush;
...
void readFile(int channelCount, bool bulkRead, bool relativeCoords, bool randomChannels,
    ...
    for (int i = 0; i < channelCount + fillChannels; i++) data[i].resizeErase(height, width);
    ...
    frameBuffer.insertSampleCountSlice(
        Slice(IMF::UINT, (char*)(&localSampleCount[0][0] - memOffset), sizeof(unsigned int) * 1,
              sizeof(unsigned int) * width, 1, 1, 0, relativeCoords, relativeCoords));
    ...
    for (int i = 0; i < channelCount; i++) {
        ...
        if (!randomChannels || read_channel[i] == 1) {
            ...
            frameBuffer.insert(str, DeepSlice(type, (char*)(&data[i][0][0] - memOffset),
                                              pointerSize * 1, pointerSize * width, sampleSize, 1,
                                              1, 0, relativeCoords, relativeCoords));
            channels_added++;
        }
    }
    if (channels_added == 0) {
    ...
    for (int i = 0; i < fillChannels; ++i) {
        ...
        ss << i << "fill";
        string str = ss.str();
        frameBuffer.insert(str, DeepSlice(type, (char*)(&data[i + channelCount][0][0] - memOffset),
                                          pointerSize * 1, pointerSize * width, sampleSize, 1, 1, 0,
                                          relativeCoords, relativeCoords));
    }
    ...
    for (int ly = 0; ly < file.numYLevels(); ly++)
        for (int lx = 0; lx < file.numXLevels(); lx++) {
            ...
            if (bulkRead) {
                ...
                file.readPixelSampleCounts(0, file.numXTiles(lx) - 1, 0, file.numYTiles(ly) - 1, lx,
                                           ly);
                    ...
                    for (int j = 0; j < file.numXTiles(lx); j++) {
                        ...
                        for (int y = box.min.y; y <= box.max.y; y++)
                            for (int x = box.min.x; x <= box.max.x; x++) {
                                int dwy = y - dataWindowL.min.y;
                                int dwx = x - dataWindowL.min.x;
                                assert(localSampleCount[dwy][dwx] ==
                                       sampleCountWhole[ly][lx][dwy][dwx]);

                                for (size_t k = 0; k < channelTypes.size(); k++) {
                                    if (channelTypes[k] == 0)
                                        data[k][dwy][dwx] =
                                            new unsigned int[localSampleCount[dwy][dwx]];
                                    if (channelTypes[k] == 1)
                        ...
                        for (int j = 0; j < file.numXTiles(lx); j++) {
                                ...
                                for (int x = box.min.x; x <= box.max.x; x++) {
                                    ...
                                    for (size_t k = 0; k < channelTypes.size(); k++) {
                                        if (!randomChannels || read_channel[k] == 1) {
                                            if (channelTypes[k] == 0)
                                                data[k][ty][tx] =
                                                    new unsigned int[localSampleCount[ty][tx]];
                                            if (channelTypes[k] == 1)
                                                data[k][ty][tx] =
                                                    new half[localSampleCount[ty][tx]];
                                            if (channelTypes[k] == 2)
                                                data[k][ty][tx] =
                            ...
                            for (int y = box.min.y; y <= box.max.y; y++)
                                for (int x = box.min.x; x <= box.max.x; x++) {
                                    ...
                                    for (size_t k = 0; k < channelTypes.size(); k++) {
                                        if (!randomChannels || read_channel[k] == 1) {
                                            checkValue(data[k][ty][tx], localSampleCount[ty][tx],
                                                       channelTypes[k], dwx, dwy);
                                            if (channelTypes[k] == 0)
                                                delete[] (unsigned int*)data[k][ty][tx];
                                            if (channelTypes[k] == 1)
