...
void readDeepScanlineFile(const char filename[], Box2i& displayWindow, Box2i& dataWindow,
    ...
    frameBuffer.insertSampleCountSlice(
        Slice(UINT, (char*)(&sampleCount[0][0] - dataWindow.min.x - dataWindow.min.y * width),
              sizeof(unsigned int) * 1,        // xStride
              sizeof(unsigned int) * width));  // yStride

    frameBuffer.insert(
        "dataZ",
        DeepSlice(FLOAT, (char*)(&dataZ[0][0] - dataWindow.min.x - dataWindow.min.y * width),
    ...
    frameBuffer.insert(
        "dataA",
        DeepSlice(HALF, (char*)(&dataA[0][0] - dataWindow.min.x - dataWindow.min.y * width),
...
void getPixelSampleData(int i, int j, Array2D<float*>& dataZ, Array2D<half*>& dataA) {
    // Dummy code creating deep data from a flat image
    dataZ[i][j][0] = testDataZ[i][j];
    dataA[i][j][0] = testDataA[i][j];
}

void writeDeepScanlineFile(const char filename[], Box2i displayWindow, Box2i dataWindow,
                           Array2D<float*>& dataZ,
...

{
    ...
    frameBuffer.insert(
        "Z", DeepSlice(FLOAT, (char*)(&dataZ[0][0] - dataWindow.min.x - dataWindow.min.y * width),
    ...
    frameBuffer.insert(
        "A", DeepSlice(HALF, (char*)(&dataA[0][0] - dataWindow.min.x - dataWindow.min.y * width),
                       sizeof(half*) * 1,      // xStride for pointer array
                       sizeof(half*) * width,  // yStride for pointer array
