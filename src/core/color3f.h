...
struct Color3f {
    ...
    Color3f clamp(float min = 0.0f, float max = 1.0f) const {
        return Color3f(std::clamp(r, min, max), std::clamp(g, min, max), std::clamp(b, min, max));
    }
...
inline Color3f operator*(float t, const Color3f &c) { return Color3f(t * c.r, t * c.g, t * c.b); }

inline Color3f operator*(const Color3f &c, float t) { return t * c; }

inline Color3f operator/(const Color3f &c, float t) { return c * (1.0 / t); }
