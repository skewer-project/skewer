...
static char* makeBasePointer(T* data, int minX, int minY, int width) {
    return reinterpret_cast<char*>(
        data - minX - static_cast<ptrdiff_t>(minY) * width
    );
    // static_cast<ptrdiff_t> ensures the memory offset calculation happens in 64-bit space, preventing overflow
    // for very large resolution images (may be unecessary but it's a good safeguard)
}

// This is computed at compile-time when possible
static constexpr int getPixelTypeSize(Imf_3_1::PixelType type) {
    switch (type) {
        case Imf_3_1::UINT:  return sizeof(unsigned int);
        case Imf_3_1::HALF:  return sizeof(Imath_3_1::half);
        case Imf_3_1::FLOAT: return sizeof(float);
        default: throw std::runtime_error("Unknown PixelType");;
    }
}

// Helper to insert a deep slice with consistent parameters
static void insertDeepSlice(Imf::DeepFrameBuffer& fb,
                     const char* name,
                     void** ptrs,
                     Imf_3_1::PixelType pixelType,
                     int minX, int minY, int width) {
    fb.insert(
        name,
        Imf::DeepSlice(
            pixelType,
            makeBasePointer(ptrs, minX, minY, width),
            POINTER_SIZE,
            POINTER_SIZE * width,
...
static bool isDeepEXR(const std::string& filename) noexcept {
 ...
 DeepImageBuffer ImageIO::LoadEXR(const std::string filename) {
     ...
     // Region for which pixel data are available is defined by a second axis-parallel rectangle in pixel space.
     ...
     // Imath::Box2i displayWindow = header.displayWindow(); // don't really need it here
     ...
     std::clog << "    Resolution: " << std::to_string(width) << "x" << std::to_string(height) << std::endl;
     ...
     //
     // We can't use the simpler setFrameBuffer interface for this part easily without constructing a partial FrameBuffer,
     // but reading sample counts is a specific operation in DeepScanLineInputFile.
     //
     ...
     if (hasZBack) {
         insertDeepSlice(frameBuffer, "ZBack", (void**)&((*zBackPtrs)[0][0]), Imf_3_1::FLOAT, minX, minY, width);
     }

     file.setFrameBuffer(frameBuffer);

     // Read the pixels, filling each ptr with the corresponding channel info per sample, based on the slices
     file.readPixels(dataWindow.min.y, dataWindow.max.y);

     // Populate DeepImageBuffer
     DeepImageBuffer deepbuf(width, height);

     // NOTE: If we wanted to be super efficient, we could make DeepImageBuffer wrap these vectors directly,
     // but DeepImageBuffer currently uses Array-of-Structures (vector<DeepSample>), so we must copy/convert.
     for (int y = 0; y < height; y++)
     {
         for (int x = 0; x < width; x++)
         {
