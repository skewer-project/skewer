...
inline shared_ptr<material> convert_obj_material(
    ...
    const std::string& base_path)
{
    ...
    std::clog << "    Kd=" << fmt_color(mtl.diffuse[0], mtl.diffuse[1], mtl.diffuse[2])
              ...
              << std::endl;
    std::clog << "    Ns=" << mtl.shininess
              << " Ni=" << mtl.ior
              << " d=" << mtl.dissolve
              << " illum=" << mtl.illum
              << std::endl;
    std::clog << "    Pm=" << mtl.metallic
              << " Pr=" << mtl.roughness
              << std::endl;
    ...
    if (emission_intensity > 0.001)
    {
        ...
        return make_shared<diffuse_light>(
            color(mtl.emission[0], mtl.emission[1], mtl.emission[2])
        );
    }
    ...
    if (spec_intensity > 0.5 && mtl.metallic < 0.001)
    {
        ...
        color albedo(mtl.diffuse[0], mtl.diffuse[1], mtl.diffuse[2]);
        std::clog << "    -> metal (traditional specular=" << spec_intensity
                  << ", fuzz=" << fuzz << ")" << std::endl;
        return make_shared<metal>(albedo, fuzz);
    }
...
inline shared_ptr<hittable> load_obj(
    ...
    shared_ptr<material> default_mat = nullptr)
{
    ...
    bool success = tinyobj::LoadObj(
        ...
        filename.c_str(), base_path.empty() ? nullptr : base_path.c_str(),
        true  // triangulate
    );
    ...
    auto fallback_mat = default_mat ? default_mat
    ...
    for (const auto& shape : shapes)
    {
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++)
        {
            ...
            bool has_normals = (idx0.normal_index >= 0) &&
                               (idx1.normal_index >= 0) &&
                               (idx2.normal_index >= 0);

            // Check if we have texture coordinates
            bool has_texcoords = (idx0.texcoord_index >= 0) &&
            ...
            if (has_normals && has_texcoords)
            {
                ...
                triangles.add(make_shared<triangle>(v0, v1, v2, face_mat, n0, n1, n2, uv0, uv1, uv2));
            }
            else if (has_normals)
            {
    ...
    std::clog << "Loaded OBJ: " << filename << " with "
              << triangles.objects.size() << " triangles" << std::endl;
