...
inline bool IntersectSphere(const Ray& r, const Sphere& s, float t_min, float t_max,
    ...
    float phi = std::atan2(-outward_normal.z(), outward_normal.x()) + MathConstants::kPi;
    si->uv = Vec3(phi / (2.0f * MathConstants::kPi), theta / MathConstants::kPi, 0.0f);

    // dpdu: partial derivative of sphere surface w.r.t. u (phi direction)
    // dpdu = 2pi * r * (nz, 0, -nx)
    si->dpdu = Vec3(2.0f * MathConstants::kPi * s.radius * outward_normal.z(), 0.0f,
                    -2.0f * MathConstants::kPi * s.radius * outward_normal.x());
    ...
    if (sin_theta > 1e-5f) {
        si->dpdv = Vec3(
            -MathConstants::kPi * s.radius * outward_normal.y() * outward_normal.x() / sin_theta,
            MathConstants::kPi * s.radius * sin_theta,
            -MathConstants::kPi * s.radius * outward_normal.y() * outward_normal.z() / sin_theta);
    } else {
        ...
        si->dpdu = Normalize(Cross(axis, outward_normal)) * (2.0f * MathConstants::kPi * s.radius);
        si->dpdv = Normalize(Cross(outward_normal, si->dpdu)) * (MathConstants::kPi * s.radius);
    }
