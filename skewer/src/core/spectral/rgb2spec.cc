...
RGB2Spec* rgb2spec_load(const char* filename) {
    ...
    size_t size_scale = sizeof(float) * m->res,
           size_data = sizeof(float) * m->res * m->res * m->res * 3 * RGB2SPEC_N_COEFFS;
...
RGB2Spec* rgb2spec_load_memory(const unsigned char* buffer, size_t length) {
    ...
    // Check "SPEC" header
    if (memcmp(buffer + offset, "SPEC", 4) != 0) {
        return NULL;
    }
    offset += 4;
    ...
    // Read resolution
    memcpy(&m->res, buffer + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    size_t size_scale = sizeof(float) * m->res;
    size_t size_data = sizeof(float) * m->res * m->res * m->res * 3 * RGB2SPEC_N_COEFFS;

    // Safety check to prevent buffer overruns
    if (offset + size_scale + size_data > length) {
        free(m);
        return NULL;
    }
