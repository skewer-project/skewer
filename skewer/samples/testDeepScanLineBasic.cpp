...
void generateRandomFile(const std::string filename, int channelCount, Compression compression,
                        bool bulkWrite, const Box2i& dataWindow, const Box2i& displayWindow) {
            ...
            for (int j = 0; j < width; j++) {
                ...
                for (int k = 0; k < channelCount; k++) {
                    if (samples > 0) {
                        ...
                        if (channelTypes[k] == 1) data[k][i][j] = new half[sampleCount[i][j]];
                        if (channelTypes[k] == 2) data[k][i][j] = new float[sampleCount[i][j]];
...
enum ReadType { eReadBulk = 1, eReadScanline, eReadScanlinelFrameBuffer };

void readFile(const std::string& filename, int channelCount, ReadType readType,
              bool randomChannels) {
    if (randomChannels) {
    ...
    if (readType == eReadBulk) {
        ...
        for (int i = 0; i < dataWindow.max.y - dataWindow.min.y + 1; i++) {
            for (int j = 0; j < width; j++) assert(localSampleCount[i][j] == sampleCount[i][j]);
            ...
            for (int j = 0; j < width; j++) {
                for (int k = 0; k < channelCount; k++) {
                    if (localSampleCount[i][j] > 0 && (!randomChannels || read_channel[k] == 1)) {
                        ...
                        if (channelTypes[k] == 1) data[k][i][j] = new half[localSampleCount[i][j]];
                        if (channelTypes[k] == 2) data[k][i][j] = new float[localSampleCount[i][j]];
                    } else {
                        data[k][i][j] = nullptr;
                    }
                }
                        ...
                        if (channelTypes[k] == 0) {
                            ...
                            if (value[l] != static_cast<unsigned int>(i * width + j) % 2049)
                                cout << j << ", " << i << " error, should be "
                                     << (i * width + j) % 2049 << ", is " << value[l] << endl
                                     << flush;
...
void readWriteTest(const std::string& tempDir, int channelCount, int testTimes,
    ...
    cout << "Testing files with " << channelCount << " channels " << testTimes << " times." << endl
         << flush;
...
void testDeepScanLineBasic(const std::string& tempDir) {
    try {
        ...
        const Box2i largeDataWindow(
            ...
            V2i(large::minX + large::width - 1, large::minY + large::height - 1));
        const Box2i largeDisplayWindow(
            V2i(0, 0), V2i(large::minX + large::width * 2, large::minY + large::height * 2));

        readWriteTest(tempDir, 1, 3, largeDataWindow, largeDisplayWindow);

        const Box2i dataWindow(V2i(small::minX, small::minY), V2i(small::minX + small::width - 1,
                                                                  small::minY + small::height - 1));
        const Box2i displayWindow(
            V2i(0, 0), V2i(small::minX + small::width * 2, small::minY + small::height * 2));
